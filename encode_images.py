...
def mainnnn(source_dir,Genrated_dir,latent_dir):
    ...
    args.generated_images_dir = Genrated_dir
    args.dlatent_dir=latent_dir
...
#
#
# def mainEncoder(source_dir,Genrated_dir,latent_dir):
...
#     parser.add_argument('--model_url', default='karras2019stylegan-ffhq-1024x1024.pkl',
#                         help='Fetch a StyleGAN model to train on from this URL')
#     parser.add_argument('--architecture', default='vgg16_zhang_perceptual.pkl',
...
#     parser.add_argument('--decay_rate', default=0.9, help='Decay rate for learning rate', type=float)
#     parser.add_argument('--iterations', default=100, help='Number of optimization steps for each batch', type=int)
#     parser.add_argument('--decay_steps', default=4,
#                         help='Decay steps for learning rate decay (as a percent of iterations)', type=float)
#     parser.add_argument('--early_stopping', default=True, help='Stop early once training stabilizes', type=str2bool,
#                         nargs='?', const=True)
#     parser.add_argument('--early_stopping_threshold', default=0.5, help='Stop after this threshold has been reached',
#                         type=float)
#     parser.add_argument('--early_stopping_patience', default=10, help='Number of iterations to wait below threshold',
#                         type=int)
...
#     parser.add_argument('--use_preprocess_input', default=True,
#                         help='Call process_input() first before using feed forward net', type=str2bool, nargs='?',
#                         const=True)
#     parser.add_argument('--use_best_loss', default=True, help='Output the lowest loss value found as the solution',
#                         type=str2bool, nargs='?', const=True)
#     parser.add_argument('--average_best_loss', default=0.25,
#                         help='Do a running weighted average with the previous best dlatents found', type=float)
#     parser.add_argument('--sharpen_input', default=True, help='Sharpen the input images', type=str2bool, nargs='?',
#                         const=True)
#
#     # Loss function options
...
#     parser.add_argument('--use_mssim_loss', default=200, help='Use MS-SIM perceptual loss; 0 to disable, > 0 to scale.',
#                         type=float)
#     parser.add_argument('--use_lpips_loss', default=100, help='Use LPIPS perceptual loss; 0 to disable, > 0 to scale.',
#                         type=float)
#     parser.add_argument('--use_l1_penalty', default=0.5, help='Use L1 penalty on latents; 0 to disable, > 0 to scale.',
#                         type=float)
...
#
#     # Generator params
#     parser.add_argument('--randomize_noise', default=False, help='Add noise to dlatents during optimization',
#                         type=str2bool, nargs='?', const=True)
...
#
#     # Masking params
#     parser.add_argument('--load_mask', default=False, help='Load segmentation masks', type=str2bool, nargs='?',
#                         const=True)
#     parser.add_argument('--face_mask', default=True, help='Generate a mask for predicting only the face area',
#                         type=str2bool, nargs='?', const=True)
#     parser.add_argument('--use_grabcut', default=True,
#                         help='Use grabcut algorithm on the face mask to better segment the foreground', type=str2bool,
#                         nargs='?', const=True)
#     parser.add_argument('--scale_mask', default=1.4, help='Look over a wider section of foreground for grabcut',
#                         type=float)
#     parser.add_argument('--composite_mask', default=True, help='Merge the unmasked area back into the generated image',
#                         type=str2bool, nargs='?', const=True)
#     parser.add_argument('--composite_blur', default=8, help='Size of blur filter to smoothly composite the images',
#                         type=int)
#
#     # Video params
#     parser.add_argument('--video_dir', default='videos', help='Directory for storing training videos')
...
#         Ddiscriminator_network=discriminator_network
#         return Ggenerator,Ddiscriminator_network
#     if (args.dlatent_avg != ''):
#         generator.set_dlatent_avg(np.load(args.dlatent_avg))
#
#     perc_model = None
