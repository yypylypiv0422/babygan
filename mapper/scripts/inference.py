	...
	for input_batch in tqdm(dataloader):
		...
		with torch.no_grad():
			...
			for i in range(len(selected_description_tuple_list)):
				...
				torchvision.utils.save_image(result_batch[2][0].unsqueeze(0), os.path.join(out_path_results, "inputl.jpg"), normalize=True, range=(-1, 1))
				torchvision.utils.save_image(result_batch[0][0].unsqueeze(0), os.path.join(out_path_results, "outputl.jpg"), normalize=True, range=(-1, 1))
